...
BOOL CreateTrayIcon(HWND hwnd) {
    ...
    if (!g_nid.hIcon) {
        // Method 2: Load from resources embedded in the exe
...
// Show balloon notification (simplified approach)
void ShowBalloonTip(const char* title, const char* text, DWORD icon) {
    g_nid.uFlags = NIF_INFO | NIF_ICON;
    g_nid.dwInfoFlags = icon;  // Use the passed icon type (NIIF_INFO, NIIF_WARNING, etc.)
    strcpy_s(g_nid.szInfoTitle, sizeof(g_nid.szInfoTitle), "");  // Empty title to reduce redundancy
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (hMutex == NULL || GetLastError() == ERROR_ALREADY_EXISTS) {
        ...
        if (existingWindow) {
            ...
            nid.uFlags = NIF_INFO;
            nid.dwInfoFlags = NIIF_INFO;  // Use standard info icon since we can't easily get custom icon
            strcpy_s(nid.szInfoTitle, sizeof(nid.szInfoTitle), "");  // Empty title
