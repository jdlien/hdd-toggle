...
cl.exe /nologo /O2 /MT /EHsc hdd-control-gui.cpp /Fe:hdd-control-gui.exe hdd-icon.res shell32.lib advapi32.lib user32.lib comctl32.lib wbemuuid.lib ole32.lib oleaut32.lib setupapi.lib /link /SUBSYSTEM:WINDOWS

if exist hdd-control-gui.obj del hdd-control-gui.obj >nul 2>nul
if exist hdd-icon.res del hdd-icon.res >nul 2>nul

if %errorlevel% equ 0 (
    echo SUCCESS! Built hdd-control-gui.exe with icon
    if exist hdd-control-gui.exe for %%f in (hdd-control-gui.exe) do echo   hdd-control-gui.exe - %%~zf bytes
) else (
